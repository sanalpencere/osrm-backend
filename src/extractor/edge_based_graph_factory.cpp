    ...
    const std::string &turn_penalties_index_filename)
{
...

    {
        ...
        std::mutex merge_mutex; // To control merging of all the data into the main data structures
        std::uint64_t nodes_completed = 0;

        const NodeID node_count = m_node_based_graph->GetNumberOfNodes();
        NodeID current_node = 0;

        tbb::filter_t<void, NodeID> intersection_ids(tbb::filter::serial_in_order,
                                                     [&](tbb::flow_control &fc) -> NodeID {
        ...
        tbb::filter_t<NodeID, std::shared_ptr<IntersectionData>> intersection_handler(
            tbb::filter::parallel, [&](const NodeID node_at_center_of_intersection) {

                if (node_at_center_of_intersection == node_count)
                    return std::make_shared<IntersectionData>();
                     ...
                     m_node_based_graph->GetAdjacentEdgeRange(node_at_center_of_intersection))
                {
                    ...
                    buffer->turn_indexes.reserve(intersection.size());
                    buffer->edges_list.reserve(intersection.size());
                    buffer->turn_weight_penalties.reserve(intersection.size());
                    buffer->turn_duration_penalties.reserve(intersection.size());
                    buffer->turn_data_container.reserve(intersection.size());
        ...
        std::vector<lookup::TurnIndexBlock> turn_indexes_buffer;
        turn_indexes_buffer.reserve(TURN_INDEX_WRITE_BUFFER_SIZE);

        tbb::filter_t<std::shared_ptr<IntersectionData>, void> buffer_storage(
            tbb::filter::serial_in_order, [&](const std::shared_ptr<IntersectionData> buffer) {
                ...
                turn_indexes_buffer.insert(turn_indexes_buffer.end(),
                                           buffer->turn_indexes.begin(),
                                           buffer->turn_indexes.end());

                // Buffer writes to reduce syscall count
                if (turn_indexes_buffer.size() == TURN_INDEX_WRITE_BUFFER_SIZE)
                {
                    turn_penalties_index_file.WriteFrom(turn_indexes_buffer.data(),
                                                        turn_indexes_buffer.size());
                    turn_indexes_buffer.clear();
                }
            });
        if (!turn_indexes_buffer.empty())
        {
            turn_penalties_index_file.WriteFrom(turn_indexes_buffer.data(),
                                                turn_indexes_buffer.size());
            turn_indexes_buffer.clear();
        ...
        tbb::parallel_pipeline(tbb::task_scheduler_init::default_num_threads() * 5,
                               intersection_ids & intersection_handler & buffer_storage);
